...
%The components are then recombined to get the file interpolation.
...
% [filt,hilb]=HighFilt_Order(temp_desaw,fr,fr,1);
...
tofix2=logical(conv(tofix,ones(1,ceil(fr*.5/2)),'same'));
power_trace(tofix2)=NaN;

power_trace_temp=power_trace;
power_trace_temp(tofix2)=interp1(find(~tofix2),power_trace(~tofix2),find(tofix2),'linear','extrap');

[filt2,hilb2]=customfilter(power_trace_temp,fr,filt_cut);
...
% breaks=find(diff(angle_angle_rel)<-pi)+1;
% for b=1:length(breaks)
%     angle_angle_rel(breaks(b):end)=angle_angle_rel(breaks(b):end)+2*pi;
% end
...
% angle_interpolated2_fixed=mod(angle_interpolated2_fixed,2*pi);
% qs=quantile(angle_interpolated2_fixed,0:.01:1);
% angle_interpolated2_fixed=interp1((0:.01:1)*2*pi,qs,angle_interpolated2_fixed,'linear','extrap');
% b=[cos(angle_interpolated2_fixed) ones(size(power_trace_temp))]\(power_trace_temp-movmean(power_trace,ceil(fr*2),'omitnan'));
% power_trace_fix=(cos(angle_interpolated2_fixed)*b(1)+b(2))+movmean(power_trace,ceil(fr*2),'omitnan');
baseline=movmax(movmin(power_trace,ceil(fr),'omitnan'),ceil(fr),'omitnan');
power_trace_fix=angle_interpolated2_fixed+baseline;
% b=movmin(movmax(power_trace_fix-baseline,fr),fr)./movmin(movmax((power_trace-baseline),fr,'omitnan'),fr);
% b=[power_trace_fix ones(size(power_trace_temp))]\(power_trace_temp);
% power_trace_fix=(power_trace_fix-baseline)./b+baseline;
...
% [filt3,hilb3]=customfilter(resid,fr,filt_cut);
% angle_interpolated3=interpolate_angles(hilb3,tofix,offset);
% 
% %determine relationship between the power envelope and periodicity of the
% %signal by regression
% angle_angle_rel=MeanSmooth_3D_circular([angle_interpolated],[angle_interpolated3],-pi-pi/40:pi/20:pi+pi/40);
% breaks=find(diff(angle_angle_rel)<-pi)+1;
% for b=1:length(breaks)
%     angle_angle_rel(breaks(b):end)=angle_angle_rel(breaks(b):end)+2*pi;
% end
% angle_interpolated3_fixed=interp1(-pi:pi/20:pi,angle_angle_rel(1:end-1),angle_interpolated);
% b=[cos(angle_interpolated3_fixed) ones(size(resid))]\(resid-movmean(resid,ceil(fr*.15)));
% resid_fix=cos(angle_interpolated3_fixed)*b(1)+b(2)+movmean(resid,ceil(fr*.15));
% resid(tofix)=resid_fix(tofix);
...
Y_all=newfilt+resid;
% Y=X;
% Y(tofix)=interp1(find(~tofix),Y(~tofix),find(tofix),'linear');
...
% power_trace(isnan(thermal_data.temperature_nostril))=NaN;
angle_sym(tofix)=NaN;
...
%     angle_interpolated(max(starts(a)-1-offset,1):ends(a)+1+offset)=newangles;
